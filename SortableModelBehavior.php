...
class SortableModel extends CActiveRecordBehavior {
    ...
    public function beforeSave($event) {
        if ($this->owner->isNewRecord) {
            $number = ($data = $this->createCommand()->select(new CDbExpression('MAX(`'.$this->orderField.'`)'))->andWhere($this->condition, $this->params)->queryScalar()) > 0
                ? $data + 1
                : 1;
    ...
    public function afterDelete($event) {
        $criteria = new CDbCriteria(array(
            ...
            'params' => array_merge(array(':position' => $this->owner->attributes[$this->orderField]), $this->params),
    ...
    public function next() {
        $this->owner->dbCriteria->mergeWith(array(
            'condition' => '`'.$this->orderField.'` = :position' . ($this->condition ? ' AND ' . $this->condition : ''),
            'params' => array_merge(array(':position' => $this->owner->attributes[$this->orderField] + 1), $this->params)
        ));
        return $this->owner;
    }
